...
- Elastic Beanstalk CLI v3.0

- A RDS database running Postgres.

- A S3 bucket for hosting uploaded web front end
...
```json
{
  ...
  "scripts": {
    ...
    "build-dev": "npm install . && npm run clean && tsc && cp -rf src/config www/config && cp -R .elasticbeanstalk www/.elasticbeanstalk && cp .npmrc www/.npmrc && cp package.json www/package.json && cd www && zip -r Archive.zip . && cd .."
  },
  "keywords": [],
